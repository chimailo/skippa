...
export default function Customers({
    ...
    const fetchCustomers = async () => {
      try {
        ...
        if (error.data.name === "UnauthorizedError") {
          ...
          router.push(`/login?callbackUrl=/customers?${search}`);
          return;
        }
        setError(error.data.message || "Failed to fetch customers");
      } finally {
        setLoading(false);
      }
    };
  ...
  async function searchCustomers() {
    try {
      ...
      setError(
        error.data.message ||
          `No customer name match the search term '${search}'`
      );
    } finally {
      setSearching(false);
    }
  ...
  return (
    ...
    >
      {loading ? (
        ...
        <FetchError message="Failed to fetch customers" />
      ) : (
